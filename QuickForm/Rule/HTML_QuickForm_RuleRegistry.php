...
class HTML_QuickForm_RuleRegistry
{
     ...
     */
    protected function _getJsValue(
    ...
    ): array
    {
        ...
        if (is_a($element, 'html_quickform_group'))
        {
            ...
            $value .= "};\n" . '  value' . $jsIndex . " = new Array();\n" . "  var valueIdx = 0;\n" .
                ...
                "[valueIdx++] = _element.options[_element.selectedIndex].value;\n" . "          }\n" .
        ...
        elseif ($element instanceof HTML_QuickForm_select || $element instanceof HTML_QuickForm_autocomplete)
        {
            if ($element->getMultiple())
            {
                ...
                $value = '  value' . $jsIndex . " = new Array();\n" . "  var valueIdx = 0;\n" .
                    ...
                    "[valueIdx++] = frm.elements['" . $elementName . "'].options[i].value;\n" . "    }\n" . "  }\n";
            }
            else
            {
                $value = '  value' . $jsIndex . " = frm.elements['" . $elementName .
                    "'].selectedIndex == -1? '': frm.elements['" . $elementName . "'].options[frm.elements['" .
                    $elementName . "'].selectedIndex].value;\n";
            }
            if ($reset)
            {
        ...
        elseif ($element->getType() == 'checkbox')
        {
            ...
            {
                $value = '  value' . $jsIndex .
                    " = frm.elements['$elementName'][1].checked? frm.elements['$elementName'][1].value: frm.elements['$elementName'][0].value;\n";
    ...
    public function getValidationScript($element, string $elementName, array $ruleData): string
    {
        ...
        {
            $js =
                $jsValue . "\n" . $jsPrefix . '  if (' . str_replace('{jsVar}', 'value', $jsCheck) . " && !errFlag['" .
